...
class Categories extends React.Component {
  ...
  async componentDidMount() {
    ...
    try {
      let response = await agent.get('/video_categories').query(params);
  ...
  async getAllCategories() {
    ...
    try {
      const response = await agent.get('/video_categories').query(params);
      categories = response.body.results;
      total = response.body.meta.totalCount;
    } catch (e) {
      notification.open({
  ...
  async handleClickDelete(id) {
    ...
    try {
      await agent.delete(`/video_categories/${id}`);
  ...
  async handleSubmitForm(event) {
    ...
    validateFields(async (err, values) => {
      ...
      const { categories, pagination, thumbnail } = this.state;
      const { name, slug, parent, description } = values;
      ...
      try {
        await agent.post('/video_categories').send(params);
  ...
  async handleClickApply() {
    ...
    try {
      await agent.delete('/video_categories').send(params);
  ...
  render() {
    const { parents, categories, thumbnail, loading } = this.state;
              ...
              <Row>
                <Col md={5}>
                  <Link
                    ...
                    to={'/videos/categories/edit-category/' + record.id}
                  >
          ...
          <Col xs={24} md={20}>
            ...
            <Form layout="vertical" onSubmit={this.handleSubmitForm}>
              <FormItem label="Name" extra="The name is how it appears on your site.">
                ...
                })(<Input onBlur={this.handleBlurTitle} />)}
              </FormItem>
              <FormItem
                label="Slug"
