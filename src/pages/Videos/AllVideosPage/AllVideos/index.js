...
class AllVideos extends React.Component {
  ...
  async componentDidMount() {
    ...
    // fetch count
    try {
      const response = await agent.get('/videos/count');
    ...
    // Fetch videos
    try {
      let response = await agent.get('/videos').query(params);
      ...
      notification.open({
        type: 'error',
        message: 'Error',
        description: 'Can not get list of videos',
      });
    }
  ...
  async findPost(keywords = '', page = 1) {
    this.setState({ loading: true });

    let total = 0;
    let videos = [];

    const params = { keywords, page, isAdmin: true };

    try {
      let response = await agent.get('/search/videos').query(params);
      total = response.body.meta.totalCount;
      videos = response.body.results;
    } catch (e) {
      notification.open({
  ...
  async handleTableChange(pagination, filters, sorter) {
    ...
    this.setState({ params: param }, async () => {
      if (!isEmpty(keywords)) {
        await this.findPost(keywords, page);
      } else {
        await this.getAllVideos();
      }
    });
  ...
  async handleClickChangeStatus(id, status) {
    let { videos, count } = this.state;
    let listOfPost = videos;
    let message = '';
    let updater = count;
    ...
    try {
      ...
      switch (status) {
        ...
        case 'pending':
        case 'private':
          await agent.put(`/videos/${id}/status/trash`);
          message = 'Move to trash successfully';
          updater[status]--;
          updater['trash']++;
          break;
        case 'restore':
          await agent.put(`/videos/${id}/status/draft`);
  ...
  async getvideosByCategory(categoryId) {
    ...
    let { videos, params } = this.state;
    let listOfPost = videos;
    let total = 0;

    try {
      let response = await agent.get(`/categories/${categoryId}/videos`).query(params);
  ...
  async getvideosByDate(timestamp) {
  ...
  async handleClickFilter() {
    ...
    if (category === 'all') {
      ...
      await this.getvideosByCategory(category);
    }
  }

  handleRowChange(selectedRowKey) {
    this.setState({
      videosId: selectedRowKey,
    });
  }
  ...
  async handleClickApply() {
    let { videos, videosId, action } = this.state;
    ...
    try {
      await agent.delete('/videos').send(params);
      videosId.forEach(id => {
        remove(listOfvideos, post => post.id === id);
      });
    } catch (e) {
      notification.open({
        type: 'error',
  ...
  openPreviewTab(id) {
    window.open(process.env.REACT_APP_WEB_URL + `/video/preview/${id}`);
  }
  ...
  render() {
    const { categories, videos, loading, count, pagination, params } = this.state;
        ...
        render: (text, record) => {
          ...
          return (
            <div>
              ...
              <Link
                to={'/videos/edit-video/' + record.id}
                className={cn({
                  'disable-link': isLock,
                })}
              >
            ...
            <Can do="manage" on={{ __type: 'CONTRIBUTOR', assignee: this.props.role }}>
              <div className="editable-row-operations">
                <Row>
                  ...
                  {get(params, 'where.status') !== 'trash' && (
                    <Col md={5}>
                      <a className="text-info" onClick={() => this.openPreviewTab(record.slug)}>
                  ...
                  {!isLock && (
                    <Col md={5}>
                      <Link className="text-warning" to={'/videos/edit-video/' + record.id}>
    ...
    return (
      <Row>
        <div className="utils__title utils__title--flat mb-3">
          <span className="text-uppercase font-size-16">videos</span>
          <Can do="manage" on={{ __type: 'AUTHOR', assignee: this.props.role }}>
            <Link to="/videos/add-new">
